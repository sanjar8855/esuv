                    ...
                    <form action="{{ route('neighborhoods.update', $neighborhood->id) }}" method="POST">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Mahallani Tahrirlash: {{ $neighborhood->name }}</h4>
                            </div>
                            <div class="card-body">
                                @if ($errors->any())
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="company_id" id="company_id"
                                        ...
                                        @foreach($companies as $company)
                                            <option
                                                value="{{ $company->id }}" {{ old('company_id', $neighborhood->company_id) == $company->id ? 'selected' : '' }}>
                                                {{ $company->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <small class="form-hint">Agar bu yerda kompaniya tanlanmasa va tanlangan shaharning
                                        o'z kompaniyasi bo'lsa, mahalla o'sha shahar kompaniyasiga biriktiriladi (yoki
                                        avvalgidek qoladi). Agar shahar ham kompaniyasiz bo'lsa, mahalla ham
                                        kompaniyasiz bo'ladi.</small>
                                    @error('company_id')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="city_id" id="city_id"
                                        ...
                                        @foreach($cities as $city)
                                            <option
                                                value="{{ $city->id }}" {{ old('city_id', $neighborhood->city_id) == $city->id ? 'selected' : '' }}>
                                                {{ $city->name }}
